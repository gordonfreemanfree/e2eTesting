...
export { SnarkyLayerStruct1, SnarkyLayerStruct2 };

import { Circuit, Field, Struct, UInt64, matrixProp } from 'snarkyjs';

import { SnarkyTensor } from './snarkyTensor.js';

// await isReady;
// create a layer
class SnarkyLayerStruct1 extends Struct({
  // @matrixProp(Field, 64, 10) weights: Array<Field>[]; // weights
  weights: Circuit.array(Circuit.array(Field, 10), 64), // weights
  // activation: Function, // activation function
  //   alpha: Field(10), // alpha value for leaky relu / it is scaled by 1000
  ...
  constructor(
    weights: Array<Field>[]
    //   weights: Circuit.array(Circuit.array(Field, 64), 10),
  ...
  exp_part(x: UInt64, y: number, z: number): UInt64 {
    ...
    for (let i = 1; i < y; i++) {
      ...
      result = result.mul(x).div(UInt64.from(Field(10000000)));
...
class SnarkyLayerStruct2 extends Struct({
  // @matrixProp(Field, 10, 10) weights: Array<Field>[]; // weights
  weights: Circuit.array(Circuit.array(Field, 10), 10),
  // activation: Function, // activation function
